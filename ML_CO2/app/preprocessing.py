...
from app.feature_engineering import feature_selection_continuous_columns
...
def preprocess(X, models_dir, training_mode):
    ...
    if training_mode:
       ...
       create_model(X_Selected, models_dir)
        ...
        #EXTRACT THE FEATURE LIST FROM TXT FILE STORED IN TRAINING PIPLINE
        a_file = open("feature_selected_continuous_columns.txt", "r")
        list_of_lists = []
        for line in a_file:
        ...
        _ = list_of_lists.remove("CO2 Emissions(g/km)")
        X_Selected = X_preprocessed[list_of_lists]

        #LOADING THE FEATURES TO MODEL
